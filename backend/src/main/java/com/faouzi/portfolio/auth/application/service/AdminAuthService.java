...
import com.faouzi.portfolio.auth.domain.model.PasswordResetToken;
...
public class AdminAuthService {
    ...
    public AuthenticatedAdmin login(String email, String password, HttpServletRequest request) {
        ...
        if (adminCandidate.isEmpty()
                || !adminCandidate.get().isEnabled()
                || !passwordEncoder.matches(password, adminCandidate.get().getPasswordHash())) {
        ...
        activityLogService.record(admin, "ADMIN_LOGIN_SUCCESS", "admin_user", admin.getId().toString(), "SUCCESS", request);
    ...
    public PasswordResetRequestResult requestPasswordReset(String email, HttpServletRequest request) {
        String normalizedEmail = AdminUser.normalizeEmail(email);
        Optional<AdminUser> adminCandidate = users.findByEmail(normalizedEmail).filter(AdminUser::isEnabled);
        ...
        if (adminCandidate.isPresent()) {
            ...
            activityLogService.record(admin, "ADMIN_PASSWORD_RESET_REQUESTED", "admin_user", admin.getId().toString(),
                    "SUCCESS", request);
        } else {
    ...
    public void resetPassword(String token, String newPassword, HttpServletRequest request) {
        ...
        if (!resetToken.usableAt(now)) {
            activityLogService.record(resetToken.getAdminUser(), "ADMIN_PASSWORD_RESET_EXPIRED", "admin_user",
                    resetToken.getAdminUser().getId().toString(), "FAILURE", request);
        ...
        activityLogService.record(admin, "ADMIN_PASSWORD_RESET_COMPLETED", "admin_user", admin.getId().toString(),
                "SUCCESS", request);
    }
    ...
    public void logout(String email, HttpServletRequest request) {
        users.findByEmail(AdminUser.normalizeEmail(email))
                .ifPresent(admin -> activityLogService.record(admin, "ADMIN_LOGOUT", "admin_user",
                        admin.getId().toString(), "SUCCESS", request));
    ...
    private AuthenticatedAdmin toAuthenticatedAdmin(AdminUser admin) {
        return new AuthenticatedAdmin(admin.getId(), admin.getEmail(), admin.isPasswordChangeRequired());
    }
}
